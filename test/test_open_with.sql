...
BEGIN
    ...
    OPEN p_rc FOR
        ...
        WITH /* A: dept 집계 CTE */
            dept_stats AS (
                SELECT /* B: dept 집계 */
                    ...
                    COUNT(*) AS cnt,
                    AVG(sal) AS avg_sal,
                    SUM(NVL(comm, /* C: NULL→0 */ 0)) AS sum_comm
                FROM emp
                GROUP BY deptno
            )
        SELECT
        ...
        LEFT JOIN dept_stats ds
               ON /* G: join 조건 */ ds.deptno = d.deptno
        ORDER BY d.deptno;
    ...
    OPEN p_rc FOR
        WITH /* H: 다중 CTE */
            base AS (
                SELECT
                    e.empno, e.ename, e.job, e.deptno, e.sal,
                    -- [I] 분석 RANK
                    RANK() OVER (
                        /* J: partition spec */
            ...
            grp_base AS ( -- [K] 집계 CTE
                SELECT
                    ...
                    SUM(
                        /* L: 1등 급여만 합산 */
                        CASE WHEN rnk = 1 THEN sal ELSE 0 END
                    ) AS top_sal
                FROM base
                GROUP BY deptno, job
            )
        ...
        ORDER BY gid, deptno NULLS LAST, job NULLS LAST;
        ...
        FROM (
            ...
            WITH
                x AS (
                    ...
                    FROM paid p
                    JOIN amounts a
                      ON /* R: join key */ a.order_id = p.order_id
                    WHERE a.amt > /* S: threshold */ 50
                )
            SELECT
                x.*,
                (
    ...
    OPEN p_rc FOR
        WITH r (node_id, parent_id, node_name, lvl, path, cycle_flag) AS (
            ...
            SELECT
                ...
                node_name,
                1 AS lvl,
                CAST(/* Z: 초기 경로 */ node_name AS VARCHAR2(4000)) AS path,
                0 AS cycle_flag
            FROM tree_nodes
            ...
            SELECT
                ...
                END AS cycle_flag
            FROM tree_nodes t
            JOIN r ON /* AD: 부모-자식 */ t.parent_id = r.node_id
            WHERE r.cycle_flag = /* AE: 사이클 없는것만 */ 0
        )
        SELECT
            node_id,
    ...
    OPEN p_rc FOR
        WITH
            recent_orders AS (
                SELECT
                    ...
                    order_dt
                FROM order_hdr
                WHERE order_dt >= SYSDATE - /* AT: 기간 */ 90
            )
        SELECT
            o.order_id,
            o.cust_name,
        ...
        OUTER APPLY (
            ...
            FROM order_item oi
            WHERE oi.order_id = /* AY: correlated */ o.order_id
        ) x
        ORDER BY o.order_id;
    ...
    OPEN p_rc FOR
        WITH
            ...
            enriched AS (
                SELECT
                    b.*,
                    NVL(b.comm, /* AZ: comm NULL→0 */ 0) AS eff_comm,
                    -- [BA] 3단 CASE 시작
                    CASE
                        WHEN b.job = 'PRESIDENT' THEN
                            CASE
                                WHEN b.sal > /* BB: elite 기준 */ 4000
                                    THEN 'ELITE'
                                ELSE 'SENIOR'
                            END
                        WHEN b.job IN (
                            ...
                            CASE
                                ...
                                WHEN b.sal >= /* BD: mid 기준 */ 2000 THEN 'MID'
                                ELSE 'LOW'
                            END
                        ELSE
                            CASE
                                WHEN NVL(b.comm, /* BE: NULL→0 */ 0) > 0
                                    THEN 'COMMISSION'
                                ELSE 'BASE'
                            END
        ...
        SELECT
            e.empno, e.ename, e.job, e.deptno, e.sal, e.pay_tier,
            /* BG: 부서명 스칼라서브쿼리 */
            (
                SELECT d.dname
                FROM dept d
    ...
    OPEN p_rc FOR
        ...
        MODEL
            ...
            RULES (
                ...
                bonus_pool[ANY] = ROUND(
                    sum_sal[CV()] * /* BM: 보너스율 */ 0.10,
                    2
                ),
                efficiency[ANY] = ROUND(
                    NULLIF(max_sal[CV()], /* BN: 0 방지 */ 0)
                    / NULLIF(sum_sal[CV()], 0)
                    * 100,
                    2
                )
        ...
        MATCH_RECOGNIZE (
            ...
            DEFINE
                ...
                B AS B.sal > PREV(B.sal)
                   AND B.sal < (
                       /* BS: 부서 평균 서브쿼리 */
                       SELECT AVG(sal) * /* BT: 상한 배율 */ 1.5
                       FROM emp
                       WHERE deptno = /* BU: correlated */ B.deptno
                   )
        )
        FETCH FIRST /* BV: 상위 20건 */ 20 ROWS ONLY;
