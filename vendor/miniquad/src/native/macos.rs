    ...
    F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
    ...
    let mut display = Box::new(MacosDisplay {
    ...
    let mut window_masks = if conf.borderless {
    ...
    if conf.window_resizable && !conf.borderless {
    ...
    if conf.borderless {
        ...
        let () = msg_send![window, setHasShadow: YES];
        ...
        if layer != nil {
            ...
            let () = msg_send![layer, setCornerRadius: 18.0f64];
        }
    }
    ...
    if screen != nil {
        ...
        let origin = NSPoint {
            x: visible_frame.origin.x + (visible_frame.size.width - window_frame.size.width) * 0.5,
            y: visible_frame.origin.y
                + (visible_frame.size.height - window_frame.size.height) * 0.5,
        };
