...
mod connection_tests {
    ...
    #[tokio::test]
    #[ignore = "requires Oracle database"]
    async fn test_interrupt_running_plsql_block() {
        let conn = connect().await.expect("Failed to connect");
        let interrupt_conn = Arc::new(conn);
        let worker_conn = Arc::clone(&interrupt_conn);

        let worker = tokio::spawn(async move {
            worker_conn
                .execute("BEGIN DBMS_LOCK.SLEEP(30); END;", &[])
                .await
        });
        ...
        interrupt_conn
            .interrupt()
            .expect("interrupt should be sent successfully");

        let err = worker
            .await
            .expect("worker task should finish")
            .expect_err("sleep block should be interrupted");

        assert!(
            err.to_string().contains("ORA-01013"),
            "expected ORA-01013 after interrupt, got: {err}"
        );
