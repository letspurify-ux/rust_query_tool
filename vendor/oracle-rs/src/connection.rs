...
pub struct Connection {
    ...
    closed: AtomicBool,
    id: u32,
    /// Cancel flag: set to true to interrupt a running query
...
impl Connection {
    ...
    ///
    /// Sets an internal cancel flag checked between row fetches. Also sends a
    /// TNS MARKER (BREAK) packet on the existing TCP socket to interrupt the
    /// server-side operation as quickly as possible.
    ///
    /// After cancellation, the connection may be in an indeterminate state.
    /// Call `reset_cancel()` before reusing the connection.
    pub fn request_cancel(&self) {
        self.cancel_requested.store(true, Ordering::Relaxed);
    }

    /// Clear the cancel flag, allowing the connection to be reused.
    ...
    /// Returns a cloneable cancel handle (Arc) for cross-thread cancellation.
