...
pub struct PlsqlResult {
    ...
    /// Original bind parameter indexes for each value in `out_values`
    pub out_param_indices: Vec<usize>,
    /// Number of rows affected (if applicable)
    pub rows_affected: u64,
    /// Cursor ID for the executed PL/SQL statement itself.
    ///
    /// This is distinct from any REF CURSOR returned in an OUT parameter.
    pub statement_cursor_id: u16,
...
impl PlsqlResult {
    ...
    pub fn empty() -> Self {
        Self {
            out_values: Vec::new(),
            out_param_indices: Vec::new(),
            rows_affected: 0,
            statement_cursor_id: 0,
            cursor_id: None,
...
pub struct ServerInfo {
    ...
    /// Whether server requires OOB check during handshake
...
struct ConnectionInner {
    ...
    sequence_number: u8,
    /// Server cursor IDs to close on the next execute round-trip
    pending_close_cursors: Vec<u16>,
    /// Statement cache for prepared statement reuse
    statement_cache: Option<StatementCache>,
}

struct InterruptTransport {
    stream: StdMutex<std::net::TcpStream>,
    supports_oob: AtomicBool,
    large_sdu: AtomicBool,
}

impl InterruptTransport {
    fn from_stream(stream: TokioTcpStream) -> Result<(TokioTcpStream, Self)> {
        let std_stream = stream.into_std().map_err(Error::Io)?;
        std_stream.set_nonblocking(true).map_err(Error::Io)?;
        let interrupt_stream = std_stream.try_clone().map_err(Error::Io)?;
        let stream = TokioTcpStream::from_std(std_stream).map_err(Error::Io)?;

        Ok((
            stream,
            Self {
                stream: StdMutex::new(interrupt_stream),
                supports_oob: AtomicBool::new(false),
                large_sdu: AtomicBool::new(false),
            },
        ))
    }

    fn update_capabilities(&self, supports_oob: bool, large_sdu: bool) {
        self.supports_oob.store(supports_oob, Ordering::Relaxed);
        self.large_sdu.store(large_sdu, Ordering::Relaxed);
    }

    fn send_interrupt(&self) -> Result<()> {
        if self.supports_oob.load(Ordering::Relaxed) && self.send_oob_break().is_ok() {
            return Ok(());
        }
        self.send_marker(crate::constants::MarkerType::Interrupt as u8)
    }

    fn send_oob_break(&self) -> Result<()> {
        let stream = self
            .stream
            .lock()
            .map_err(|_| Error::Internal("interrupt stream mutex poisoned".to_string()))?;
        send_all_with_flags(&stream, &[0x21], platform_msg_oob()).map_err(Error::Io)
    }

    fn send_marker(&self, marker_type: u8) -> Result<()> {
        let large_sdu = self.large_sdu.load(Ordering::Relaxed);
        let mut packet = Vec::with_capacity(PACKET_HEADER_SIZE + 3);
        let packet_len = (PACKET_HEADER_SIZE + 3) as u32;
        ...
        packet.push(PacketType::Marker as u8);
        packet.push(0);
        packet.extend_from_slice(&[0, 0]);
        packet.extend_from_slice(&[1, 0, marker_type]);

        let stream = self
            .stream
            .lock()
            .map_err(|_| Error::Internal("interrupt stream mutex poisoned".to_string()))?;
        send_all_with_flags(&stream, &packet, 0).map_err(Error::Io)
    }
}

#[cfg(unix)]
fn platform_msg_oob() -> i32 {
    libc::MSG_OOB
}

#[cfg(windows)]
fn platform_msg_oob() -> i32 {
    windows_sys::Win32::Networking::WinSock::MSG_OOB as i32
}

fn send_all_with_flags(stream: &std::net::TcpStream, data: &[u8], flags: i32) -> io::Result<()> {
    let mut sent = 0usize;
    let mut retry_count = 0usize;

    while sent < data.len() {
        match platform_send(stream, &data[sent..], flags) {
            Ok(0) => {
                return Err(io::Error::new(
                    io::ErrorKind::WriteZero,
                    "failed to send interrupt bytes",
                ));
            }
            Ok(written) => {
                sent += written;
...
#[cfg(unix)]
fn platform_send(stream: &std::net::TcpStream, data: &[u8], flags: i32) -> io::Result<usize> {
...
fn platform_send(stream: &std::net::TcpStream, data: &[u8], flags: i32) -> io::Result<usize> {
    ...
    use windows_sys::Win32::Networking::WinSock::{
        send, WSAGetLastError, SOCKET, SOCKET_ERROR, WSAEINTR, WSAEWOULDBLOCK,
    };
    ...
    if result == SOCKET_ERROR {
        ...
        let kind = match code {
            WSAEWOULDBLOCK => io::ErrorKind::WouldBlock,
            WSAEINTR => io::ErrorKind::Interrupted,
            _ => io::ErrorKind::Other,
        };
        Err(io::Error::new(
            kind,
            format!("winsock send failed with code {code}"),
        ))
    } else {
        Ok(result as usize)
    }
...
impl ConnectionInner {
    ...
    fn queue_cursor_for_close(&mut self, cursor_id: u16) {
        if cursor_id == 0 || self.pending_close_cursors.contains(&cursor_id) {
            return;
        }
        self.pending_close_cursors.push(cursor_id);
    }

    fn take_pending_close_cursors(&mut self) -> Vec<u16> {
        std::mem::take(&mut self.pending_close_cursors)
    }
    ...
    async fn send_marker(&mut self, marker_type: u8) -> Result<()> {
        ...
        let payload_len = 3usize; // 0x01, 0x00, marker_type
        ...
        if self.large_sdu {
            ...
            buf.write_u16_be(0)?; // checksum / reserved
        }
...
fn configure_execute_message_with_pending_closes(
    inner: &mut ConnectionInner,
    execute_msg: &mut ExecuteMessage<'_>,
) {
    if let Some(ref mut cache) = inner.statement_cache {
        for cursor_id in cache.take_cursors_to_close() {
            inner.queue_cursor_for_close(cursor_id);
        }
    }

    let close_cursor_ids = inner.take_pending_close_cursors();
    if !close_cursor_ids.is_empty() {
        let close_seq_num = inner.next_sequence_number();
        execute_msg.set_close_cursors(close_seq_num, close_cursor_ids);
    }
...
async fn send_execute_message(
    inner: &mut ConnectionInner,
    execute_msg: &ExecuteMessage<'_>,
) -> Result<()> {
    let large_sdu = inner.large_sdu;
    let message = execute_msg.build_message_only(&inner.capabilities)?;
    let max_single_packet_payload = inner
        .sdu_size
        .saturating_sub((PACKET_HEADER_SIZE + 2) as u16)
        as usize;

    if message.len() > max_single_packet_payload {
...
pub struct Connection {
    ...
    config: Config,
    closed: AtomicBool,
    break_in_progress: AtomicBool,
    id: u32,
    interrupt_transport: Option<Arc<InterruptTransport>>,
}
...
impl Connection {
    ...
    pub async fn connect_with_config(config: Config) -> Result<Self> {
        ...
        let tcp_stream = TokioTcpStream::connect(&addr).await?;

        // Set TCP options
        tcp_stream.set_nodelay(true)?;

        // Wrap with TLS if configured
        let (stream, interrupt_transport) = if config.is_tls_enabled() {
            ...
            (OracleStream::Tls(tls_stream), None)
        } else {
            let (tcp_stream, interrupt_transport) = InterruptTransport::from_stream(tcp_stream)?;
            (
                OracleStream::Plain(tcp_stream),
                Some(Arc::new(interrupt_transport)),
            )
        };
        ...
        let conn = Connection {
            ...
            break_in_progress: AtomicBool::new(false),
            id,
            interrupt_transport,
        };
        ...
        conn.sync_interrupt_transport().await;
    ...
    async fn sync_interrupt_transport(&self) {
        let Some(interrupt_transport) = self.interrupt_transport.as_ref() else {
            return;
        };

        let inner = self.inner.lock().await;
        interrupt_transport.update_capabilities(inner.server_info.supports_oob, inner.large_sdu);
    }
    ...
    async fn perform_handshake(&self) -> Result<()> {
        ...
        let needs_oob_check = {
            ...
            inner.server_info.protocol_version >= crate::constants::version::MIN_OOB_CHECK
                ...
                && inner.server_info.supports_oob_check
        };
        if needs_oob_check {
    ...
    async fn send_oob_check(&self) -> Result<()> {
        if let Some(interrupt_transport) = self.interrupt_transport.as_ref() {
            interrupt_transport.send_oob_break()?;
        }

        if let Some(interrupt_transport) = self.interrupt_transport.as_ref() {
            let _ = interrupt_transport;
        } else {
        ...
        loop {
            ...
            match packet_type {
                2 => {
                    ...
                    inner.server_info.supports_oob_check = accept.supports_oob_check;
                    inner.sdu_size = accept.sdu.min(65535) as u16;
                    if let Some(interrupt_transport) = self.interrupt_transport.as_ref() {
                        interrupt_transport
                            .update_capabilities(inner.server_info.supports_oob, inner.large_sdu);
                    }
    ...
    pub async fn execute_plsql(&self, sql: &str, params: &[BindParam]) -> Result<PlsqlResult> {
        ...
        let mut inner = self.inner.lock().await;
        configure_execute_message_with_pending_closes(&mut inner, &mut execute_msg);
        send_execute_message(&mut inner, &execute_msg).await?;

        // Receive response
        let response = self.receive_interruptible_packet(&mut inner).await?;
        if response.len() <= PACKET_HEADER_SIZE {
    ...
    pub async fn execute_batch(&self, batch: &BatchBinds) -> Result<BatchResult> {
        ...
        // Receive the complete response. DML/DDL acknowledgements can span
        // multiple TNS DATA packets and leaving trailing packets unread causes
        // later executions to see stale protocol bytes.
        let response = self.receive_interruptible_response(&mut inner).await?;
        if response.len() <= PACKET_HEADER_SIZE {
    ...
    fn parse_batch_response(
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            if self.parse_common_response_message(&mut buf, msg_type, &mut end_of_response)? {
                continue;
            }
            ...
            match msg_type {
                ...
                x if x == MessageType::Error as u8 => {
                    ...
                    if error_code != 0 && error_code != 1403 && error_code != 24344 {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected batch response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    async fn execute_query_with_params(
        ...
        if has_lob_columns && !statement.requires_define() {
            ...
            configure_execute_message_with_pending_closes(&mut inner, &mut define_msg);

            send_execute_message(&mut inner, &define_msg).await?;

            // Receive the re-execute response
            let define_response = self.receive_interruptible_packet(&mut inner).await?;
    ...
    fn parse_query_response_with_columns(
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected query response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    fn parse_plsql_response(
            ...
            match msg_type {
                ...
                x if x == MessageType::RowData as u8 => {
                    if !out_columns.is_empty() {
                        let row = self.parse_plsql_out_row_data(
                            &mut buf,
                            &out_columns,
                            caps,
                            out_bit_vector.as_deref(),
                        )?;
                        out_bit_vector = None;
                ...
                // BitVector (21) - column presence bitmap for sparse OUT bind rows
                x if x == MessageType::BitVector as u8 => {
                    let _num_columns_sent = buf.read_ub2()?;
                    let num_bytes = (out_columns.len() + 7) / 8;
                    if num_bytes > 0 {
                        out_bit_vector = Some(buf.read_bytes_vec(num_bytes)?);
                    }
                }
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected PL/SQL response message type {}",
                        msg_type
                    )));
                }
            }
        ...
        Ok(PlsqlResult {
            out_values,
            out_param_indices: _out_indices,
            rows_affected: row_count,
            statement_cursor_id,
            cursor_id,
            implicit_results,
    ...
    /// Skip keyword/value pairs returned by the server.
    ///
    /// The TTC payload first sends the pair lengths as UB2 values and then the
    /// actual bytes in Oracle's inner length-prefixed/chunked format.
    fn skip_keyword_value_pairs(&self, buf: &mut ReadBuffer, num_pairs: u16) -> Result<()> {
    ...
    /// Skip a TTC byte sequence encoded as UB4 outer length + inner
    /// length-prefixed/chunked bytes.
    fn skip_outer_length_prefixed_bytes(&self, buf: &mut ReadBuffer) -> Result<()> {
        let num_bytes = buf.read_ub4()?;
        if num_bytes > 0 {
            buf.skip_raw_bytes_chunked()?;
        }
    ...
    /// Parse and discard warning info. Warnings are informational for our
    /// execution flow, but the payload must still be consumed to keep the
    /// stream aligned.
    fn parse_warning_info(&self, buf: &mut ReadBuffer) -> Result<()> {
        let _error_num = buf.read_ub2()?;
        let num_bytes = buf.read_ub2()?;
        buf.skip_ub2()?;

        if num_bytes > 0 {
            let _warning = buf.read_string_with_length()?;
        }

        Ok(())
    }

    /// Parse and discard server-side piggyback payloads.
    ///
    /// Oracle sends these out-of-band session updates alongside ordinary
    /// responses. If they are not consumed exactly, subsequent message parsing
    /// becomes desynchronized.
    fn parse_server_side_piggyback(&self, buf: &mut ReadBuffer) -> Result<()> {
        const PIGGYBACK_QUERY_CACHE_INVALIDATION: u8 = 1;
        const PIGGYBACK_OS_PID_MTS: u8 = 2;
        const PIGGYBACK_TRACE_EVENT: u8 = 3;
        const PIGGYBACK_SESS_RET: u8 = 4;
        const PIGGYBACK_SYNC: u8 = 5;
        const PIGGYBACK_LTXID: u8 = 7;
        const PIGGYBACK_AC_REPLAY_CONTEXT: u8 = 8;
        const PIGGYBACK_EXT_SYNC: u8 = 9;
        const PIGGYBACK_SESS_SIGNATURE: u8 = 10;

        let opcode = buf.read_u8()?;

        match opcode {
            PIGGYBACK_LTXID => {
                self.skip_outer_length_prefixed_bytes(buf)?;
            }
            PIGGYBACK_QUERY_CACHE_INVALIDATION | PIGGYBACK_TRACE_EVENT => {}
            PIGGYBACK_OS_PID_MTS => {
                let num_bytes = buf.read_ub2()?;
                if num_bytes > 0 {
                    buf.skip_raw_bytes_chunked()?;
                }
            ...
            PIGGYBACK_SYNC => {
                ...
                buf.skip_ub1()?;
                self.skip_keyword_value_pairs(buf, num_elements)?;
                buf.skip_ub4()?;
            }
            PIGGYBACK_EXT_SYNC => {
                buf.skip_ub2()?;
    ...
    /// Consume response message types that can appear in many execution paths.
    fn parse_common_response_message(
        &self,
        buf: &mut ReadBuffer,
        msg_type: u8,
        end_of_response: &mut bool,
    ) -> Result<bool> {
        match msg_type {
            x if x == MessageType::Warning as u8 => {
                self.parse_warning_info(buf)?;
                Ok(true)
            }
            x if x == MessageType::Token as u8 => {
                let _token = buf.read_ub8()?;
                Ok(true)
            }
            x if x == MessageType::ServerSidePiggyback as u8 => {
                self.parse_server_side_piggyback(buf)?;
                Ok(true)
            }
            x if x == MessageType::EndOfResponse as u8 => {
                *end_of_response = true;
    ...
    fn parse_return_parameters_internal(
        ...
        self.skip_keyword_value_pairs(buf, num_pairs)?;
    ...
    /// Parse PL/SQL OUT bind row data.
    ///
    /// Oracle appends a per-bind status byte after each returned value in
    /// PL/SQL OUT bind row data. Query row data does not include this extra
    /// byte, so it needs a dedicated decoder here.
    fn parse_plsql_out_row_data(
        &self,
        buf: &mut ReadBuffer,
        columns: &[ColumnInfo],
        caps: &Capabilities,
        ...
        for (col_idx, col) in columns.iter().enumerate() {
            let value_is_present = if let Some(bv) = bit_vector {
                let byte_num = col_idx / 8;
                let bit_num = col_idx % 8;
                byte_num < bv.len() && (bv[byte_num] & (1 << bit_num)) != 0
            } else {
                true
            };
            ...
            let value = self.parse_column_value(buf, col, caps)?;
            values.push(value);
            // The extra per-bind status byte observed on Oracle thin OUT bind
            // rows is emitted as 0x00. Do not blindly skip any remaining byte
            // here, because the final value may be followed immediately by the
            // next message type (for example FlushOutBinds/Error), which would
            // desynchronize the connection.
            if buf.remaining() > 0 && buf.peek_u8()? == 0 {
                buf.skip(1)?;
            }
        }
    ...
    fn parse_dml_response(&self, payload: &[u8]) -> Result<QueryResult> {
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected DML response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    pub async fn clear_statement_cache(&self) {
        ...
        inner.pending_close_cursors.clear();
    }

    /// Queue a server cursor to be closed on the next execute round-trip.
    pub async fn queue_cursor_for_close(&self, cursor_id: u16) {
        if cursor_id == 0 || self.is_closed() {
            return;
        }
    ...
    /// Flush any queued server cursor closes immediately.
    pub async fn flush_pending_close_cursors(&self) -> Result<()> {
        self.ensure_ready().await?;

        let mut inner = self.inner.lock().await;
        self.flush_pending_close_cursors_inner(&mut inner).await
    }
    ...
    fn parse_lob_read_response(&self, payload: &[u8], locator: &LobLocator) -> Result<LobData> {
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected LOB read response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    pub async fn create_temp_lob(&self, oracle_type: OracleType) -> Result<LobLocator> {
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected CREATE_TEMP LOB response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    fn parse_lob_bool_response(&self, payload: &[u8], locator: &LobLocator) -> Result<bool> {
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected LOB bool response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    fn parse_lob_simple_response(&self, payload: &[u8], locator: &LobLocator) -> Result<()> {
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected LOB simple response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    fn parse_lob_amount_response(&self, payload: &[u8], locator: &LobLocator) -> Result<u64> {
        ...
        while !end_of_response && buf.remaining() > 0 {
            ...
            match msg_type {
                ...
                _ => {
                    return Err(Error::Protocol(format!(
                        "Unexpected LOB amount response message type {}",
                        msg_type
                    )));
                }
            }
    ...
    /// Interrupt the current operation from another thread.
    ///
    /// This follows the Oracle thin split-path more closely:
    /// use an out-of-band break when the server negotiated OOB support and
    /// always send the interrupt marker without forcing the connection closed.
    pub fn interrupt(&self) -> Result<()> {
        if self.closed.load(Ordering::Relaxed) {
            return Err(Error::ConnectionClosed);
        }
        self.break_in_progress.store(true, Ordering::Relaxed);

        match self.interrupt_transport.as_ref() {
            Some(interrupt_transport) => interrupt_transport.send_interrupt(),
            None => Err(Error::FeatureNotSupported(
                "interrupt is not supported for TLS thin connections".to_string(),
            )),
        }
    }
    ...
    async fn send_marker(&self, inner: &mut ConnectionInner, marker_type: u8) -> Result<()> {
        ...
        // Build MARKER packet header
        let packet_len = PACKET_HEADER_SIZE + 3usize; // Header + 3 bytes payload
        if inner.large_sdu {
            packet_buf.write_u32_be(packet_len as u32)?;
        } else {
            packet_buf.write_u16_be(packet_len as u16)?;
            packet_buf.write_u16_be(0)?; // checksum / reserved
        }
        packet_buf.write_u8(PacketType::Marker as u8)?;
    ...
    async fn send_simple_function_inner(
        ...
        // Wait for response
        let response = inner.receive().await?;
        self.handle_function_response(inner, function_code, response)
            .await
    }

    async fn receive_interruptible_packet(&self, inner: &mut ConnectionInner) -> Result<Bytes> {
        self.receive_interruptible(inner, false).await
    }

    async fn receive_interruptible_response(&self, inner: &mut ConnectionInner) -> Result<Bytes> {
        self.receive_interruptible(inner, true).await
    }

    async fn receive_interruptible(
        &self,
        inner: &mut ConnectionInner,
        accumulate_response: bool,
    ) -> Result<Bytes> {
        let mut response = if accumulate_response {
            inner.receive_response().await?
        } else {
            inner.receive().await?
        };

        if response.len() >= PACKET_HEADER_SIZE && response[4] == PacketType::Marker as u8 {
            response = inner.handle_marker_reset().await?;
        }

        if self.break_in_progress.swap(false, Ordering::Relaxed) {
            if inner.server_info.supports_oob {
                self.send_marker(inner, crate::constants::MarkerType::Interrupt as u8)
                    .await?;
            }
            response = if accumulate_response {
                inner.receive_response().await?
            } else {
                inner.receive().await?
            };
    ...
    async fn flush_pending_close_cursors_inner(&self, inner: &mut ConnectionInner) -> Result<()> {
        if let Some(ref mut cache) = inner.statement_cache {
            for cursor_id in cache.take_cursors_to_close() {
                inner.queue_cursor_for_close(cursor_id);
            }
        ...
        buf.write_ub4(close_cursor_ids.len() as u32)?;
        for cursor_id in close_cursor_ids {
        ...
        let response = inner.receive().await?;
        self.handle_function_response(inner, FunctionCode::CloseCursors, response)
            .await
    }

    async fn handle_function_response(
        &self,
        inner: &mut ConnectionInner,
        function_code: FunctionCode,
