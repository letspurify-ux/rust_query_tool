...
impl BindParam {
    ...
    pub fn placeholder_value(&self) -> Value {
        ...
        match self.oracle_type {
            OracleType::Varchar | OracleType::Char | OracleType::Long => Value::Null,
            OracleType::Number | OracleType::BinaryInteger => Value::Null,
            OracleType::BinaryDouble | OracleType::BinaryFloat => Value::Null,
            OracleType::Date => Value::Null,
            OracleType::Timestamp | OracleType::TimestampTz | OracleType::TimestampLtz => {
                Value::Null
            }
            OracleType::Raw | OracleType::LongRaw => Value::Null,
            OracleType::Clob | OracleType::Blob => Value::Null,
            OracleType::Cursor => Value::Null,
            OracleType::Boolean => Value::Null,
            _ => Value::Null,
        }
    }
...
impl Statement {
    ...
    fn parse_bind_variables(&mut self) {
        ...
        while i < len {
            if let Some(next) = Self::skip_line_comment(&chars, i) {
                i = next;
                continue;
            }

            if let Some(next) = Self::skip_block_comment(&chars, i) {
                i = next;
                continue;
            }

            if let Some(next) = Self::skip_q_quoted_string(&chars, i) {
                i = next;
                continue;
            }

            if let Some(next) = Self::skip_regular_string(&chars, i) {
                i = next;
                continue;
            }
    ...
    fn skip_regular_string(chars: &[char], start: usize) -> Option<usize> {
    ...
    fn skip_q_quoted_string(chars: &[char], start: usize) -> Option<usize> {
...
mod tests {
    ...
    #[test]
    fn test_binds_in_strings_with_escaped_quotes_ignored() {
        let stmt = Statement::new("SELECT * FROM t WHERE note = 'it''s :not_a_bind' AND y = :y");
        assert_eq!(stmt.bind_info().len(), 1);
        assert_eq!(stmt.bind_info()[0].name, "Y");
    }

    #[test]
    fn test_binds_in_q_quoted_strings_ignored() {
        let stmt = Statement::new(
            "INSERT INTO t (payload, note, id) VALUES (q'!{\"a\":1,\"b\":2}!', q'[text: still not a bind]', :id)",
        );
        assert_eq!(stmt.bind_info().len(), 1);
        assert_eq!(stmt.bind_info()[0].name, "ID");
    }
