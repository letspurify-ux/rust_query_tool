...
pub struct StatementCache {
    ...
    max_size: usize,
    /// Cursor IDs that should be closed on the next round-trip
    ...
    pub fn mark_cursor_closed(&mut self, sql: &str) {
        ...
        if let Some(cached) = self.cache.get_mut(sql) {
            ...
            if cursor_id != 0 {
                ...
                tracing::trace!(sql = sql, cursor_id, "Queued cursor for close");
            }
        }
        if cursor_id != 0 && !self.cursors_to_close.contains(&cursor_id) {
            self.cursors_to_close.push(cursor_id);
        }
    }

    /// Take any cursor IDs that should be closed on the next round-trip.
