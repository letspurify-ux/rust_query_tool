...
pub struct ExecuteMessage<'a> {
    ...
    sequence_number: u8,
    /// Cursor IDs to close on the server before executing this statement
    close_cursors: Vec<u16>,
    /// Sequence number for the close-cursors piggyback
    close_cursor_sequence_number: Option<u8>,
...
impl<'a> ExecuteMessage<'a> {
    ...
    pub fn new(statement: &'a Statement, options: ExecuteOptions) -> Self {
        ...
        Self {
            ...
            sequence_number: 1,
            close_cursors: Vec::new(),
            close_cursor_sequence_number: None,
            skip_row_data: false,
            bind_metadata: None,
        }
    }
    ...
    /// Set cursor IDs to piggyback-close before executing this statement.
    pub fn set_close_cursors(&mut self, seq: u8, cursor_ids: Vec<u16>) {
        self.close_cursor_sequence_number = Some(seq);
        self.close_cursors = cursor_ids;
    }
    ...
    /// Build just the execute message bytes (without packet header or data flags).
    ///
    /// This is used when the caller needs to split the request across multiple
    /// TNS DATA packets because the encoded message exceeds the negotiated SDU.
    pub fn build_message_only(&self, caps: &Capabilities) -> Result<Bytes> {
        let mut buf = WriteBuffer::new();

        if !self.close_cursors.is_empty() {
            self.write_close_cursors_piggyback(&mut buf, caps)?;
        }
    ...
    fn write_close_cursors_piggyback(
        ...
        buf.write_ub4(self.close_cursors.len() as u32)?;
        for cursor_id in &self.close_cursors {
