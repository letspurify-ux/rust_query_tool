    use super::{
        ...
        IfSymbolEvent, LineBoundaryAction, LineLeadingMarker, PendingDo, PendingEnd,
        PendingEndSuffix, RoutineFrame, SemicolonAction, SemicolonPolicy, SlashLineKind,
  ...
  FUNCTION f RETURN NUMBER IS"
    ...
    fn nested_language_dollar_quoted_targets_do_not_force_external_split() {
        for target in ["$$C$$", "$lang$JAVA$lang$", "$lang$PYTHON$lang$"] {
            ...
            assert!(
                statements[0]
                    .starts_with("CREATE OR REPLACE PROCEDURE proc_language_dollar_ident IS"),
                "first statement should keep procedure body for {target}: {}",
                statements[0]
            );
            assert!(
