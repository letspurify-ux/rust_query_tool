                                            ...
                                            match guard.binds.get_mut(&key) {
                                                Some(bind) => match &mut bind.value {
                                                    BindValue::Scalar(value) => {
                                                        PrintValue::Scalar(value.clone())
                                                    }
                                                    BindValue::Cursor(cursor_slot) => {
                                                        if let Some(cursor) = cursor_slot.take() {
                                                            PrintValue::Cursor(cursor)
                                                        } else {
                                                            PrintValue::CursorEmpty
                                                        }
                                                    }
                                                },
                                                None => PrintValue::Missing,
                                            }
                                        };

                                        match print_value {
                                            PrintValue::Scalar(value) => {
                                                let columns =
                                            ...
                                            PrintValue::Cursor(cursor) => {
                                                ...
                                                if let Some(bind) = guard.binds.get_mut(&key) {
                                                    bind.value = BindValue::Cursor(Some(cursor));
                                                }
                                            }
                                            PrintValue::CursorEmpty => {
                                                SqlEditorWidget::emit_script_message(
