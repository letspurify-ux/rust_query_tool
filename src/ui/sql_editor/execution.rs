...
impl SqlEditorWidget {
    ...
    fn mysql_connection_info_snapshot_for_ui(
        shared_connection: &crate::db::SharedConnection,
        db_activity: &str,
    ) -> Option<ConnectionInfo> {
        ...
        Some(Self::connection_info_for_ui(conn_guard.get_info()))
    }
    ...
    fn acquire_oracle_pooled_execution_connection<'a>(
        ...
        if let Some((lease, prior_may_have_uncommitted_work, prior_requires_transaction_decision)) =
            ...
            if conn.ping().is_ok() {
                ...
                match setup_result {
                    Ok(()) => {
                        return (
                            conn_guard,
                            Ok(Some((conn, preserve_existing_session_state))),
                        )
                    }
    ...
    fn start_oracle_lazy_select(
        ...
        if let Err(err) = spawn_result {
            ...
            cleanup_pooled_db_session
                .clear_oracle_if_current_connection(connection_generation, &cleanup_conn);
            ...
            .spawn(move || {
                ...
                let worker_result = panic::catch_unwind(AssertUnwindSafe(|| {
                    ...
                    *lazy_cancel_context
                        ...
                        Err(err) => {
                            ...
                            if Self::mysql_error_allows_session_reuse(&err)
                                || (Self::lazy_fetch_cancel_requested(
                                    &active_lazy_fetch,
                                    session_id,
                                ) && Self::mysql_explicit_cancel_can_reuse_session(state_hint))
                            {
                                keep_session = true;
                            }
                            let suppress_error_result =
                    ...
                    should_retain_session = keep_session
                        && Self::lazy_fetch_can_keep_session(&active_lazy_fetch, session_id)
                        && (!close_cancelled
                            || Self::mysql_explicit_cancel_can_reuse_session(state_hint));
                    ...
                    if should_retain_session {
                        should_retain_session = Self::sync_mysql_pooled_session_info(
                            &shared_connection,
                            conn,
                            "mysql lazy fetch cleanup",
                            connection_generation,
        ...
        let execute_mysql_sql =
            |sql: &str, auto_commit: bool| -> Result<Vec<QueryResult>, String> {
                ...
                let state_hint = SqlEditorWidget::mysql_batch_session_state_hint_for_sql(
                    ...
                    &mysql_batch_may_hold_table_lock,
                    &mysql_batch_may_hold_named_lock,
                    &mysql_batch_saw_uncertain_named_lock_release,
                );
                    ...
                    match command {
                        ...
                        ToolCommand::Use { database } => {
                            ...
                            match execute_mysql_sql(use_sql.as_str(), auto_commit) {
                                Ok(_) => {
                                    let info = Self::mysql_connection_info_snapshot_for_ui(
                                        shared_connection,
                                        db_activity,
                                    );
                        ...
                        )
                    {
                        match Self::acquire_mysql_pooled_session(
                            ...
                            Ok((
                                ...
                                conn,
                                prior_may_have_uncommitted_work,
                                _prior_requires_transaction_decision,
                            )) => {
                                let (heading_enabled, feedback_enabled) =
                                    SqlEditorWidget::current_output_settings(session);
    ...
    fn execute_sql_with_mysql_delimiter(
        ...
        if !self.resolve_required_transaction_decision("running the next query") {
            return;
        }
                    ...
                    match item {
                        ScriptItem::ToolCommand(command) => {
                            ...
                            match command {
                                ...
                                ToolCommand::SetAutoCommit { enabled } => {
                                    let has_dirty_transaction = oracle_prior_may_have_uncommitted_work
                                    ...
                                    if has_dirty_transaction {
                                        SqlEditorWidget::emit_script_message(
                                            &sender,
                                            &session,
                                            "SET AUTOCOMMIT",
                                            "Error: Auto-commit cannot be changed while this session has a dirty transaction.",
                                        );
                                        command_error = true;
                                    } else {
                                        let mut conn_guard = lock_connection_with_activity(
                                ...
                                ToolCommand::Use { ref database } => {
                                    ...
                                    match use_result {
                                        Ok(()) => {
                                            ...
                                            if let Some(info) = info {
                                                let _ =
                                                    sender.send(QueryProgress::DatabaseChanged {
                                                        info,
                                                    });
                                            } else {
                                                let _ = sender
    ...
    fn mysql_session_may_have_uncommitted_work<C: Queryable>(
        ...
        let sql = "\
            SELECT COUNT(CASE \
                WHEN COALESCE(trx_rows_modified, 0) > 0 \
                  OR COALESCE(trx_rows_locked, 0) > 0 \
                THEN 1 \
            END) \
    ...
    fn mysql_pooled_session_may_need_preservation<C: Queryable>(
        ...
        crate::db::mysql_session_may_need_preservation_after_statement(
            prior_may_have_uncommitted_work,
            state_hint,
    ...
    fn mysql_batch_session_state_hint_for_sql(
        ...
        batch_may_hold_table_lock: &std::cell::Cell<bool>,
        batch_may_hold_named_lock: &std::cell::Cell<bool>,
        batch_saw_uncertain_named_lock_release: &std::cell::Cell<bool>,
    ) -> MySqlSessionStateHint {
        let state_hint = SqlEditorWidget::mysql_session_state_hint_for_sql(sql);

        if crate::db::mysql_statement_releases_table_lock(sql) {
            batch_may_hold_table_lock.set(false);
        } else if crate::db::mysql_statement_acquires_table_lock(sql) {
            batch_may_hold_table_lock.set(true);
        }

        if crate::db::mysql_statement_releases_all_named_locks(sql) {
            batch_may_hold_named_lock.set(false);
            batch_saw_uncertain_named_lock_release.set(false);
        } else if crate::db::mysql_statement_acquires_named_lock(sql) {
            batch_may_hold_named_lock.set(true);
        } else if crate::db::mysql_statement_releases_named_lock(sql) {
            ...
            batch_saw_uncertain_named_lock_release.set(true);
        }

        if QueryExecutor::is_plain_commit(sql)
            || (QueryExecutor::is_plain_rollback(sql)
                && !crate::db::mysql_rollback_targets_savepoint(sql))
        {
    ...
    fn finalize_mysql_batch_pooled_session(
        ...
        let Some((lease, prior_may_have_uncommitted_work, prior_requires_transaction_decision)) =
            pooled_db_session.take_reusable_with_decision_state(
                connection_generation,
    ...
    pub(super) fn mysql_session_state_hint_for_sql(sql: &str) -> MySqlSessionStateHint {
        crate::db::mysql_session_state_hint_for_sql(sql)
    }
    ...
    fn reusable_mysql_pooled_session_is_ready(
        ...
        // A reusable MySQL/MariaDB pooled connection is already this editor
        // tab's session. Refresh encoding/session options, but do not reselect
        // the global connection database here; another tab may have changed it.
        crate::db::DatabaseConnection::apply_mysql_session_settings(conn, advanced)?;
        Ok(true)
    }
    ...
    fn acquire_mysql_pooled_session(
        ...
        let (mut conn, prior_may_have_uncommitted_work, prior_requires_transaction_decision) =
            if let Some((
                lease,
                prior_may_have_uncommitted_work,
                prior_requires_transaction_decision,
                ...
                .and_then(|(lease, prior, decision)| {
                    lease
                        .into_mysql_connection()
                        .map(|conn| (conn, prior, decision))
                })
            {
                let mut conn = lease;
        ...
        if let Err(message) = Self::apply_mysql_pooled_execution_session_settings(
            ...
            if (prior_may_have_uncommitted_work || prior_requires_transaction_decision)
                && Self::mysql_error_allows_session_reuse(&message)
            {
                Self::retain_mysql_pooled_session_if_current_with_transaction_decision(
                    shared_connection,
    ...
    fn retain_mysql_pooled_session(
        pooled_db_session: &SharedDbSessionLease,
        connection_generation: u64,
        conn: mysql::PooledConn,
    ...
    fn retain_mysql_pooled_session_if_current(
        shared_connection: &crate::db::SharedConnection,
        pooled_db_session: &SharedDbSessionLease,
        connection_generation: u64,
        conn: mysql::PooledConn,
    ...
    fn retain_mysql_pooled_session_if_current_with_transaction_decision(
        shared_connection: &crate::db::SharedConnection,
        pooled_db_session: &SharedDbSessionLease,
        connection_generation: u64,
        conn: mysql::PooledConn,
        ...
        if should_retain {
            Self::retain_mysql_pooled_session(
                pooled_db_session,
                connection_generation,
                conn,
                may_have_uncommitted_work,
    ...
    pub(super) fn apply_mysql_transaction_mode_to_reusable_pooled_session(
        ...
        if prior_may_have_uncommitted_work || prior_requires_transaction_decision {
            ...
            return Err(
                "Transaction mode cannot be changed while this tab has a dirty transaction session."
                    .to_string(),
            );
        }

        match crate::db::DatabaseConnection::apply_mysql_transaction_mode(&mut conn, mode) {
            Ok(()) => {
                Self::retain_mysql_pooled_session_if_current(
                    shared_connection,
                    pooled_db_session,
    ...
    fn mysql_pooled_action_result_after_cancel<T>(
        ...
        match result {
            Ok(Ok(_)) => Err(Self::cancel_message()),
    ...
    fn sync_mysql_pooled_session_info(
        ...
        if !Self::mysql_pooled_session_health_check(conn, db_activity) {
            return false;
        }
        let mut conn_guard =
            lock_connection_with_activity(shared_connection, db_activity.to_string());
        ...
        F: FnOnce(&mut mysql::PooledConn) -> Result<T, MysqlError>,
    {
        ...
        let interruption_requires_transaction_decision =
            Self::mysql_pooled_action_requires_transaction_decision(
                was_cancelled,
                recoverable_timeout,
                lock_wait_timeout,
                auto_commit,
        ...
        let requires_transaction_decision =
            crate::db::mysql_requires_transaction_decision_after_statement(
                prior_requires_transaction_decision,
                state_hint,
                statement_failed,
        ...
        } else if was_cancelled || recoverable_timeout {
            Self::mysql_explicit_cancel_can_reuse_session(state_hint)
                && Self::sync_mysql_pooled_session_info(
                    shared_connection,
                    &mut conn,
        ...
        Self::set_current_mysql_cancel_context(current_mysql_cancel_context, None);
        if should_retain_session {
            let fallback_on_error = if requires_transaction_decision {
                true
            } else if state_hint.clears_session_state {
                state_hint.may_leave_session_bound_state
            } else {
            ...
            Self::retain_mysql_pooled_session_if_current_with_transaction_decision(
                ...
                connection_generation,
                conn,
                may_have_uncommitted_work || requires_transaction_decision,
                requires_transaction_decision,
                log_context,
            );
        } else {
...
mod query_execution_cleanup_tests {
    use super::{
        InterruptKind, LazyFetchAllTimeout, LazyFetchCancelHandle, LazyFetchCommand,
        LazyFetchHandle, MySqlQueryCancelContext, QueryExecutionCleanupGuard, QueryProgress,
        SqlEditorWidget,
    };
    use crate::db::{
    ...
    fn mysql_explicit_cancel_reuses_only_plain_select_like_sessions() {
        let select_hint = SqlEditorWidget::mysql_session_state_hint_for_sql("SELECT 1");
        assert!(SqlEditorWidget::mysql_explicit_cancel_can_reuse_session(
            select_hint
        ));
        ...
        for sql in [
            ...
            assert!(
                !SqlEditorWidget::mysql_explicit_cancel_can_reuse_session(hint),
    ...
    fn mysql_batch_lock_hints_track_table_and_named_locks_separately() {
        ...
        assert!(batch_may_hold_table_lock.get());
    ...
    fn mysql_transaction_policy_preserves_prior_dirty_state_after_failed_clear_statement() {
        ...
        assert!(
            crate::db::mysql_session_may_need_preservation_after_statement(
                true, ddl_hint, false, true, false
            )
        );
        assert!(
            !crate::db::mysql_session_may_need_preservation_after_statement(
                true, ddl_hint, false, false, false
            )
        );
    }
    ...
    fn mysql_transaction_policy_resolves_prior_decision_after_successful_commit() {
        ...
        let unknown_hint = SqlEditorWidget::mysql_session_state_hint_for_sql("COMMIT RELEASE");
        assert!(
            crate::db::mysql_requires_transaction_decision_after_statement(
                false,
                unknown_hint,
    ...
    #[test]
    fn mysql_pooled_action_result_after_cancel_never_reports_success() {
...
mod mysql_batch_execution_regression_tests {
    ...
    #[test]
    #[ignore = "requires local MySQL or MariaDB test database via SPACE_QUERY_TEST_MYSQL_* env vars"]
    fn mysql_pooled_session_keeps_tab_database_after_global_database_changes() {
...
SELECT 'FINAL_STATUS' AS section_name,
       CASE
           WHEN DATABASE() = 'information_schema'
       ...
       END AS result_status;
",
            "mysql tab database retention verification",
        );
...
";
    ...
    #[test]
    #[ignore = "requires local MariaDB test database via SPACE_QUERY_TEST_MYSQL_* env vars"]
    fn mariadb_use_and_transaction_scripts_keep_independent_tab_sessions() {
        if !mysql_test_server_is_mariadb().unwrap_or(false) {
        ...
        let scripts = [
            "\
DROP TABLE IF EXISTS qt_tab_session_tx7;
CREATE TABLE qt_tab_session_tx7 (id INT PRIMARY KEY);
@TRANSACTION
INSERT INTO qt_tab_session_tx7 VALUES (7);
SELECT COUNT(*) AS tx7_count FROM qt_tab_session_tx7;
",
            "\
DROP TABLE IF EXISTS qt_tab_session_tx8;
CREATE TABLE qt_tab_session_tx8 (id INT PRIMARY KEY);
@TRANSACTION
INSERT INTO qt_tab_session_tx8 VALUES (8);
SELECT COUNT(*) AS tx8_count FROM qt_tab_session_tx8;
",
            "\
CREATE DATABASE IF NOT EXISTS qt_tab_session_use5;
USE qt_tab_session_use5;
SET @qt_tab_marker = 'test5';
SELECT @qt_tab_marker AS marker;
",
            "\
CREATE DATABASE IF NOT EXISTS qt_tab_session_use4;
USE qt_tab_session_use4;
SET @qt_tab_marker = 'test4';
SELECT @qt_tab_marker AS marker;
",
        ];

        for (index, script) in scripts.iter().enumerate() {
            let progress = execute_mysql_script_on_tab_slot(
                &harness.shared_connection,
                &harness.session,
            ...
            assert_no_failed_mysql_statement(&progress);
            apply_session_clearing_connection_transition_for_test(&progress, &pooled_sessions);
        }

        let snapshots = pooled_sessions
            .iter()
            .map(|pooled_session| pooled_session.snapshot())
            .collect::<Vec<_>>();
        assert!(
            snapshots.iter().all(Option::is_some),
            "each MariaDB editor tab should retain its own DB session: {snapshots:?}"
        );
        assert!(
            snapshots[0].unwrap().may_have_uncommitted_work,
            "test7-style transaction tab should be dirty"
        );
        assert!(
            snapshots[1].unwrap().may_have_uncommitted_work,
            "test8-style transaction tab should be dirty"
        );
        assert!(
            !snapshots[2].unwrap().may_have_uncommitted_work,
            "USE-only tab session should not require commit/rollback"
        );
        assert!(
            !snapshots[3].unwrap().may_have_uncommitted_work,
            "non-transaction USE tab session should not require commit/rollback"
        );

        for (index, cleanup_sql) in [
            "ROLLBACK; DROP TABLE IF EXISTS qt_tab_session_tx7;",
            "ROLLBACK; DROP TABLE IF EXISTS qt_tab_session_tx8;",
            "DROP DATABASE IF EXISTS qt_tab_session_use5;",
            "DROP DATABASE IF EXISTS qt_tab_session_use4;",
        ]
        .iter()
        .enumerate()
        {
            let _ = execute_mysql_script_on_tab_slot(
                &harness.shared_connection,
                &harness.session,
                &pooled_sessions[index],
                &auto_commit_overrides[index],
