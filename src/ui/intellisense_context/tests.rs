...
#[test]
fn phase_for_update_of_identifier_named_skip_after_comma_stays_column_context() {
    let ctx = analyze("SELECT * FROM emp FOR UPDATE OF empno, skip |");
    assert_eq!(ctx.phase, SqlPhase::SetClause);
    assert!(ctx.phase.is_column_context());
}
...
#[test]
fn phase_for_share_of_identifier_named_skip_after_comma_stays_column_context() {
...
#[test]
fn phase_for_no_key_update_of_identifier_named_skip_after_comma_stays_column_context() {
    let ctx = analyze("SELECT * FROM emp FOR NO KEY UPDATE OF empno, skip |");
    assert_eq!(ctx.phase, SqlPhase::SetClause);
    assert!(ctx.phase.is_column_context());
}

#[test]
fn phase_for_key_share_of_identifier_named_skip_after_comma_stays_column_context() {
    let ctx = analyze("SELECT * FROM emp FOR KEY SHARE OF empno, skip |");
    assert_eq!(ctx.phase, SqlPhase::SetClause);
    assert!(ctx.phase.is_column_context());
}
