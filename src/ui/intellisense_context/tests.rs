...
#[test]
fn resolve_qualifier_prefers_exact_schema_qualified_name_over_short_name_collision() {
    let tables = vec![
        ScopedTableRef {
            name: "scott.emp".to_string(),
            alias: None,
            depth: 0,
            is_cte: false,
        },
        ScopedTableRef {
            name: "hr.emp".to_string(),
            alias: None,
            depth: 0,
            is_cte: false,
        },
    ];
    let result = resolve_qualifier_tables("scott.emp", &tables);
    assert_eq!(result, vec!["scott.emp"]);
}

#[test]
fn oracle_as_of_timestamp_modifier_keeps_real_alias_in_scope() {
    let ctx = analyze("SELECT e.| FROM emp AS OF TIMESTAMP SYSTIMESTAMP e WHERE e.empno = 10");
    assert_eq!(ctx.phase, SqlPhase::SelectList);
    assert!(
        ctx.tables_in_scope
            .iter()
            .any(|t| t.alias.as_deref() == Some("e") && t.name.eq_ignore_ascii_case("emp")),
        "AS OF modifier must not consume the real alias: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_partition_modifier_keeps_real_alias_in_scope() {
    let ctx = analyze("SELECT s.| FROM sales PARTITION (p202501) s WHERE s.amount > 0");
    assert!(
        ctx.tables_in_scope
            .iter()
            .any(|t| t.alias.as_deref() == Some("s") && t.name.eq_ignore_ascii_case("sales")),
        "PARTITION modifier must not consume the real alias: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_versions_modifier_keeps_real_alias_in_scope() {
    let ctx = analyze("SELECT t.| FROM orders VERSIONS BETWEEN SCN 100 AND 200 t WHERE t.order_id > 0");
    assert!(
        ctx.tables_in_scope
            .iter()
            .any(|table| table.alias.as_deref() == Some("t") && table.name.eq_ignore_ascii_case("orders")),
        "VERSIONS modifier must not consume the real alias: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_match_recognize_clause_is_not_alias_and_keeps_column_phase() {
    let ctx = analyze(
        "SELECT * FROM sales MATCH_RECOGNIZE (PARTITION BY | ORDER BY sale_ts MEASURES CLASSIFIER() AS cls ALL ROWS PER MATCH PATTERN (A+) DEFINE A AS amount > 0)",
    );
    ...
    assert!(
        ctx.tables_in_scope
            .iter()
            .all(|table| table.alias.as_deref() != Some("MATCH_RECOGNIZE")),
        "MATCH_RECOGNIZE must not be consumed as relation alias: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_qualify_clause_maps_to_column_context() {
    let ctx = analyze(
        "SELECT empno, ROW_NUMBER() OVER (PARTITION BY deptno ORDER BY sal DESC) rn FROM emp QUALIFY |",
    );
    assert_eq!(ctx.phase, SqlPhase::WhereClause);
    assert!(ctx.phase.is_column_context());
}
...
#[test]
fn oracle_json_table_relation_alias_is_visible() {
    let ctx = analyze(
        "SELECT jt.| FROM JSON_TABLE(payload, '$' COLUMNS (id NUMBER PATH '$.id')) jt",
    );
    assert!(
        ctx.tables_in_scope
            .iter()
            .any(|table| table.alias.as_deref() == Some("jt")),
        "JSON_TABLE alias must be visible: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_xmltable_relation_alias_is_visible() {
    let ctx = analyze(
        "SELECT x.| FROM XMLTABLE('/rows/row' PASSING xml_col COLUMNS id NUMBER PATH 'id') x",
    );
    assert!(
        ctx.tables_in_scope
            .iter()
            .any(|table| table.alias.as_deref() == Some("x")),
        "XMLTABLE alias must be visible: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_table_function_relation_alias_is_visible() {
    let ctx = analyze("SELECT t.| FROM TABLE(pkg.get_rows(:p_id)) t");
    assert!(
        ctx.tables_in_scope
            .iter()
            .any(|table| table.alias.as_deref() == Some("t")),
        "TABLE(...) alias must be visible: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_dblink_table_name_is_parsed_with_alias() {
    let ctx = analyze("SELECT e.| FROM scott.emp@hr_link e WHERE e.empno = 10");
    assert!(
        ctx.tables_in_scope.iter().any(|table| {
            table.alias.as_deref() == Some("e") && table.name.eq_ignore_ascii_case("scott.emp@hr_link")
        }),
        "DBLINK relation name should be parsed as a single relation with alias: {:?}",
        ctx.tables_in_scope
    );
}

#[test]
fn oracle_pivot_for_expression_is_column_context() {
    let ctx = analyze("SELECT * FROM sales PIVOT (SUM(amount) FOR | IN ('A' AS A, 'B' AS B))");
    assert_eq!(ctx.phase, SqlPhase::PivotClause);
    assert!(ctx.phase.is_column_context());
}

#[test]
fn oracle_model_dimension_expression_is_column_context() {
    let ctx = analyze(
        "SELECT * FROM sales MODEL PARTITION BY (deptno) DIMENSION BY (|) MEASURES (sal)",
    );
