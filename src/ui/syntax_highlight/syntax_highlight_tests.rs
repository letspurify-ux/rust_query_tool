...
fn test_entry_state_in_single_quote_continues() {
    ...
    let (styles, exit) =
        highlighter.generate_styles_with_state(text, LexerState::InSingleQuote);
    assert_eq!(exit, LexerState::Normal);
...
fn test_entry_state_in_double_quote_continues() {
    ...
    let (styles, exit) =
        highlighter.generate_styles_with_state(text, LexerState::InDoubleQuote);
    assert_eq!(exit, LexerState::Normal);
...
fn test_entry_state_block_comment_never_closes() {
    ...
    let (styles, exit) =
        highlighter.generate_styles_with_state(text, LexerState::InBlockComment);
    assert_eq!(exit, LexerState::InBlockComment);
    assert!(
...
fn test_large_file_stateful_delimiter_does_not_scan_entire_buffer() {
    ...
    buf.set_text(&text);
    let ranges =
        select_highlight_ranges(&buf, text_len, cursor_pos, Some((0, text_len)), None);
    assert!(
