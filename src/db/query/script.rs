...
impl QueryExecutor {
    ...
    fn is_start_script_command(trimmed: &str) -> bool {
        ...
        let Some(second_word) = Self::next_meaningful_word(trimmed, 1) else {
            return true;
        };

        if !second_word.eq_ignore_ascii_case("WITH") {
            return true;
        }

        // Hierarchical query clause "START WITH <expr>" must stay as SQL,
        // but SQL*Plus still allows a script file literally named "with".
        Self::next_meaningful_word(trimmed, 2).is_none()
    }
    ...
    fn is_connect_command(trimmed: &str) -> bool {
        ...
        if first_word.eq_ignore_ascii_case("CONN") {
            return true;
        }
        ...
        !Self::next_meaningful_word(trimmed, 1)
            .is_some_and(|second_word| second_word.eq_ignore_ascii_case("BY"))
    }
