...
5. `flush_out_binds`가 있으면 추가 요청/응답 1회
   - 전송 바이트: `start_request(TNS_PACKET_TYPE_DATA)` + `write_uint8(TNS_MSG_TYPE_FLUSH_OUT_BINDS)` + `end_request()`
   - 이어서 `_receive_packet(message)` + `message.process(read_buf)` 재실행
...
BaseException 발생 (아래 4개 조건 모두 AND):
  - not _in_connect
  - _write_buf._packet_sent
  - _read_buf._transport is not None
  - _read_buf._transport._transport is not None
  → BREAK marker 전송
  → _reset()
  → 원래 예외 re-raise
```
...
raw(num_bytes)             (bit_vector)
```

**폴라리티**: `bit == 1`이면 해당 컬럼은 fresh(새 값), `bit == 0`이면 duplicate(직전 row 값 재사용, payload 없음). `bit_vector` 자체가 NULL이면 모든 컬럼이 fresh로 간주된다(`_is_duplicate_data` 항상 False).
...
각 `var_impl`에 대해 (분기 조건은 cursor 상태 기준):

```
if var_impl.is_array:
    ub4  num_elements_in_array
    loop num_elements_in_array:
        _process_column_data(buf, var_impl, pos)
elif cursor_impl._statement._is_returning:
    ub4  num_rows
    loop num_rows:
        _process_column_data(buf, var_impl, j)
elif cursor_impl.fetching_arrow:
    if _is_duplicate_data(i): var_impl._arrow_array.append_last_value(...)
    else: _process_column_data(buf, var_impl, row_index)
elif _is_duplicate_data(i):
    # 직전 row의 값을 그대로 복제 (아무 byte도 읽지 않음)
    value = var_impl._values[cursor_impl._last_row_index]
else:
...
row_index += 1
if in_fetch:
    cursor_impl._last_row_index = row_index - 1
    cursor_impl._buffer_rowcount = row_index
    self.bit_vector = NULL          # per-row 리셋
on_row_completed()
```

**핵심 두 가지**:
1. 분기 플래그는 `var_impl.*`이 아니라 **`cursor_impl.*` / `cursor_impl._statement.*`**. 오인하기 쉬우니 주의.
2. `bit_vector`는 **매 row마다 NULL로 리셋**된다. 즉 다음 row의 중복 정보는 반드시 그 row 직전의 `ROW_HEADER`/`BIT_VECTOR` 메시지에서 다시 받아야 한다. 이전 row의 bit_vector를 캐시해서 쓰면 안 된다.

**`_is_duplicate_data(i)` 폴라리티**: `bit_vector == NULL`이면 False(모두 fresh). 그렇지 않으면 `(bit_vector[i//8] & (1 << i%8)) == 0`일 때만 True(중복). 즉 bit 0이 duplicate, bit 1이 fresh.

## 25. `_process_column_data()` 규칙

### 25.0 메타 선행 조건

- `in_fetch`면 `var_impl._fetch_metadata`, 아니면 `var_impl.metadata`를 사용 → `ora_type_num`, `csfrm` 결정.
- `var_impl.bypass_decode`면 `ora_type_num = ORA_TYPE_NUM_RAW`로 강제.

### 25.1 선행 null 판정 (가장 먼저)

```
if metadata.buffer_size == 0 AND in_fetch
   AND ora_type_num not in (LONG, LONG_RAW, UROWID):
    column_value = None         # 바이트 0개 소비. 여기서 반환
```

이 조건이 맞으면 아래 분기/테일 전부 건너뛴다.

### 25.2 `ora_type_num`별 분기

| ora_type_num | non-fetch (out bind) | in_fetch |
| --- | --- | --- |
| `ROWID` | `read_str(CS_FORM_IMPLICIT)` | `ub1 num_bytes`; 0 또는 `TNS_NULL_LENGTH_INDICATOR`면 None, 아니면 `read_rowid` |
| `UROWID` | `read_str(CS_FORM_IMPLICIT)` | `read_urowid` |
| `CURSOR` | `ub1 (skip) length` → `_create_cursor_from_describe(buf, column_value)` → `ub2 cursor_id` | 동일 + `statement._is_nested = True` |
| `CLOB` / `BLOB` / `BFILE` | `read_lob_with_length` | 동일 |
| `JSON` | `read_oson` | 동일 |
| `VECTOR` | `read_vector` | 동일. `fetching_arrow`면 `convert_vector_to_arrow` 추가 |
| `OBJECT` | `typ_impl == None`이면 `read_xmltype`, 아니면 `read_dbobject(typ_impl)` | 동일 |
| 그 외 | `read_oracle_data(...)` | 동일. `fetching_arrow`면 `convert_oracle_data_to_arrow`, 아니면 `convert_oracle_data_to_python` |

### 25.3 테일 — 두 경로 mutually exclusive

```
if not in_fetch:
    sb4  actual_num_bytes
    if actual_num_bytes < 0 AND ora_type_num == BOOLEAN:
        column_value = None
    elif actual_num_bytes != 0 AND column_value is not None:
        raise ERR_COLUMN_TRUNCATED
elif ora_type_num in (LONG, LONG_RAW):
    sb4  (skip) null_indicator
    ub4  (skip) return_code
```

그 외(일반 in_fetch 비-LONG)에는 테일 바이트가 붙지 않는다. non-fetch와 LONG tail을 동시에 읽지 않도록 분기 순서 주의.
...
## 27. `_process_implicit_result()` 필드 맵

`DBMS_SQL.RETURN_RESULT`로 반환되는 암시적 result set(bind되지 않은 cursor)을 받는 경로.

```
ub4  num_results
cursor_impl._implicit_resultsets = []
loop num_results:
  ub1  num_bytes
  raw(num_bytes)                        (skip, 선행 블록)
  child_cursor = _create_cursor_from_describe(buf)
  ub2  cursor_id                        → child_cursor_impl._statement._cursor_id
  cursor_impl._implicit_resultsets.append(child_cursor)
```

bind 경로와 달리 `out_var_impls`를 쓰지 않고, 각 child cursor가 자체 fetch 상태를 가진다.
