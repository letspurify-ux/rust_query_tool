...
저장소 전체의 스레드/상태 공유 코드에 공통 적용한다.

- 동시성 이슈 방지를 위해 스레드 간에 공유되는 상태에서 `Rc<RefCell<T>>`/`RefCell<T>` 패턴은 금지한다.
...
### 7.1 panic 회피 코딩 가이드 (Rust)

아래 규칙은 "런타임에서 죽지 않는 앱"을 위한 최소 가이드다.

- 외부 입력(DB 응답, 파일, UI 입력, 네트워크)은 모두 실패 가능 값으로 간주하고 즉시 `Result`/`Option`으로 감싼다.
- `unwrap`/`expect` 대신 다음 패턴을 우선한다.
  - `ok_or_else(...)` + `?`로 문맥 있는 에러 변환
  - `match`/`if let`로 분기 후 복구 가능한 기본 동작 제공
  - `unwrap_or_else(...)`는 "안전한 기본값"이 명확할 때만 사용
- 인덱스 접근은 `get(...)`/`get_mut(...)`를 우선 사용하고, 슬라이스는 `is_char_boundary()` 확인 후 수행한다.
- 산술 연산 오버플로우 가능 구간은 `checked_*`/`saturating_*` 계열 API를 우선 고려한다.
- 스레드/락 처리에서 독성(poison) 가능성을 고려해 `lock()` 실패 경로를 복구 가능한 에러로 처리한다.
- 백그라운드 작업 실패는 로그만 남기고 무시하지 말고, UI/호출자에게 실패 상태를 전달한다.

권장 에러 메시지 원칙:

- "무엇을 하다가 실패했는지"를 함수/동작 단위로 포함한다.
- 사용자 표시용 메시지와 내부 디버깅 메시지(원인 에러)를 분리한다.
- 동일 에러를 상위로 전파할 때는 문맥을 1단계씩 추가한다.
...
- [ ] `unwrap`/`expect`/`panic!`이 신규 도입되지 않았고, 인덱스 슬라이싱은 경계 검증과 함께 사용되었는가?
- [ ] 실패 가능 경로(DB/파일/UI/네트워크)의 에러가 호출자 또는 UI까지 전달되는가?
- [ ] 인덱스 접근에 `get`/`get_mut` 등 안전 API를 우선 사용했는가?
- [ ] 오버플로우 가능 산술 연산에 `checked_*`/`saturating_*` 검토를 했는가?
- [ ] cargo test 모두 통과했는가?

---

## 10) AGENTS.md 자체 수정 규칙
